...
export class ApproveOrDisqualifyOpinionnComponent implements OnInit {
  ...
  pageSize: number = 9;
  length: number = 0;
  constructor(private OpinionService: OpinionService, private userService: UsersService) { }
  ...
  getServerData(event?: PageEvent) {
    this.OpinionService.getallOpinion(event.pageSize, event.pageIndex).subscribe(
      response => {
        if (response) {
  ...
  DisqualifyOpinion(opinionId: number) {
    let opinion: Opinion = this.opinionArr.find(x => x.OpinionId == opinionId);
    opinion.IsConfirmed = false;
    this.userService.sendMail(opinion.Users.Email, opinion.Summary).subscribe(res => {
      if (res) {
        this.OpinionService.updateisConfirmedOfOpinion(opinion).subscribe(res=>{
          if(res)
          {

            Swal.fire('Success', 'הסרת את חוות הדעת בהצלחה,ונשלח מייל עם הודעת פסילה למשתמש', 'success');
           this.func();
          }
